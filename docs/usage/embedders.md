...
- **Install:** `pip install nexus-memory[sentence-transformers]`. Missing → `ImportError` with that hint.
...
- **Install:** `pip install nexus-memory[openai]`. Missing → `ImportError` with that hint.
