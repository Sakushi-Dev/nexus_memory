...
The orchestrator builds the embedder from the config when you don't inject one, defaulting to a `HashingEmbedder`, so a plain run still does no model download and no network access:

```python
# src/nexus_memory/core/orchestrator.py
self.embedder: Embedder = embedder or _build_embedder(config)   # picks the config.embedder_backend
```

`_build_embedder` resolves `config.embedder_backend` (`"hashing"` default, or `"fastembed"`). You may also pass any `Embedder` instance via the `embedder` argument of [`NexusMemory`](api-reference.md) (it wins over the config). Either way a **dim guard** checks `embedder.dim` against the real stored vector dimension before any write, and **(0.7.0)** the active embedder's *provenance* (backend + model + dim) is recorded in the `system_config` table — opening a store with a different embedder than it was written with is refused (to prevent silently mixing vector spaces), pointing you at the [re-index tool](#switching-embedders--re-indexing). See [Choosing an embedder](#choosing-an-embedder-summary) below and the [API reference](api-reference.md) for the full constructor.
...
- **Config knobs (0.7.0):** `embedder_backend` (`"hashing"|"fastembed"`), `embedder_model` (override the model), `embedder_cache_dir` (model cache location), `embedder_offline` (require a warm cache; never hit the network).
- **Dimension:** probed from the model (`emb.dim` == 768 for bge-base); the dim guard enforces it against the store.
- **Offline:** the first construction downloads the model (a one-line log notes the size + cache dir); afterwards no network is used. With `embedder_offline=True` and no warm cache it raises an actionable error telling you to warm up online once.
...
For the recommended **same-dimension** path (the default `HashingEmbedder` and `bge-base-en-v1.5` are both 768), this recomputes every fact's vector and rewrites the provenance **in a single transaction** — no schema change, no data loss. Switching to a *different* dimension (e.g. a 384- or 512-d model) is a larger, schema-affecting migration and is out of scope for the bundled same-dim tool.
