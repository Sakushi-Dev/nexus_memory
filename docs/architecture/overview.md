...
- **Layer IV — Procedural** holds *directives* — standing behavioral rules ("Keep answers concise.", "Address the user as Sam."). Rules are upserted (UNIQUE on `directive`), priority-ranked (1–10), and activatable.
...
NexusMemory(
    ...
    detector=None,       # default: MockDirectiveDetector (tone + persona, offline)
