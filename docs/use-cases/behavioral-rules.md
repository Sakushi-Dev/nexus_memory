...
- **Generic always/never only as a fallback.** The `immer/always` and `nie/never` rules fire **only when nothing more specific matched**, to avoid noisy duplicate `other` rules for a sentence already captured as a tone/persona directive.
- **Stable order.** Output ordering is tone, persona, then the generic rule.
